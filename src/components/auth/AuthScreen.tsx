...
const field = 'w-full px-3 py-2.5 text-sm focus:outline-none';
...
export function AuthScreen() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="bg-white w-full max-w-md overflow-hidden" style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
        <div className="px-8 pt-8 pb-0">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-2.5" style={{ backgroundColor: 'var(--accent-primary)', borderRadius: 'var(--radius-md)' }}>
              <GraduationCap className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="font-serif text-lg" style={{ color: 'var(--text-primary)' }}>Studiegruppe Hub</h1>
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Samarbejd med din gruppe i realtid</p>
            </div>
          </div>
...
function LoginForm({ onSwitch }: { onSwitch: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(false);
    if (!result.success) setError(result.error ?? 'Fejl ved login');
  }
...
function SignupFlow({ onSwitch }: { onSwitch: () => void }) {
  const { signup, createGroup, joinGroup } = useApp();
  ...
  const filteredSchools = INSTITUTIONS.filter(s => s.toLowerCase().includes(schoolSearch.toLowerCase()));

  async function handleStep1(e: React.FormEvent) {
    e.preventDefault();
    setStep1Error(''); setSubmitting(true);
    const result = await signup(name.trim(), email.trim(), password);
    setSubmitting(false);
    if (!result.success || !result.userId) { setStep1Error(result.error ?? 'Fejl'); return; }
  ...
  async function handleStep2(e: React.FormEvent) {
    e.preventDefault();
    setStep2Error(''); setSubmitting(true);
    try {
      if (groupMode === 'create') {
        if (!school || !program.trim() || !groupName.trim()) { setStep2Error('Udfyld alle felter'); setSubmitting(false); return; }
        const code = await createGroup(pendingUserId, groupName.trim(), program.trim(), school);
        setFinalInviteCode(code);
      } else {
        const result = await joinGroup(pendingUserId, inviteInput.trim());
        if (!result.success) { setStep2Error(result.error ?? 'Fejl'); setSubmitting(false); return; }
      }
      setStep(3);
    } catch (err) {
      setStep2Error(err instanceof Error ? err.message : 'Ukendt fejl');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  const progress = [{ n: 1, label: 'Profil' }, { n: 2, label: 'Gruppe' }, { n: 3, label: 'Færdig' }];
    ...
    <div className="px-8 pb-6">
      ...
      <div className="flex items-center gap-2 mb-6">
        {progress.map(({ n, label: lbl }, i) => (
          <div key={n} className="flex items-center gap-2 flex-1">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors ${step >= n ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{n}</div>
      ...
      {step === 1 && (
        <form onSubmit={handleStep1} className="space-y-4">
          ...
          <button type="submit" disabled={!name.trim() || !email.trim() || password.length < 6 || submitting}
            ...
            {submitting ? 'Opretter…' : <><span>Næste</span><ChevronRight className="w-4 h-4" /></>}
          </button>
          <p className="text-center text-xs text-gray-400">
      ...
      {step === 2 && (
        <form onSubmit={handleStep2} className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="submit"
              disabled={submitting || (groupMode === 'create' ? (!groupName.trim() || !school || !program.trim()) : inviteInput.length !== 6)}
              ...
              {submitting ? 'Gemmer…' : groupMode === 'create' ? 'Opret gruppe' : 'Join gruppe'}
            </button>
          </div>
        </form>
      )}
